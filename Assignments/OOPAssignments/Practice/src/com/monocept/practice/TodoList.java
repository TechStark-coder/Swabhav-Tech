...
package com.monocept.practice;
	...
	public boolean readTaskFromUser() {
		...
		try {
			System.out.println(
					Messages.GREEN_TEXT + "Please enter the following details to add a task:" + Messages.RESET_TEXT);
	...
	/**
	 * A method to read the value from user (standard input, i.e., terminal) and
	 * update the given Task object in the ArrayList of Tasks
	 * 
	...
	public boolean readTaskFromUserToUpdate(Task task) {
		...
		try {
			...
			Messages.showMessage(
					"Task is " + (isTaskUpdated ? "updated successfully" : "NOT modified") + ": Returning to Main Menu",
					false);
	...
	/**
	 * A method to display the contents of ArrayList with first column as task
	 * number
	 */
	public void listAllTasksWithIndex() {
		...
		taskList.stream().forEach(task -> System.out.println(String.format(displayFormat, taskList.indexOf(task) + 1,
				task.getTitle(), task.getProject(), task.getDueDate(), (task.isComplete() ? "YES" : "NO"))));
	}
	 ...
	 *               otherwise it will sorty by date
	 */
	public void listAllTasks(String sortBy) {
		Messages.separator('=', 75);
		System.out.println("Total Tasks = " + taskList.size() + "\t\t (Completed = " + completedCount() + "\t\t"
		...
		if (sortBy.equals("2")) {
			...
			taskList.stream().sorted(Comparator.comparing(Task::getProject))
					.forEach(task -> System.out.println(String.format(displayFormat, task.getProject(), task.getTitle(),
							task.getDueDate(), (task.isComplete() ? "YES" : "NO"))));
		} else {
			...
			taskList.stream().sorted(Comparator.comparing(Task::getDueDate))
					.forEach(task -> System.out.println(String.format(displayFormat, task.getDueDate(), task.getTitle(),
							task.getProject(), (task.isComplete() ? "YES" : "NO"))));
		}
	}

	/**
	 * A method to select a particular Task object from ArrayList and perform
	 * editing operations
	 * 
	 * @param selectedTask Task number that is selected by user from given list to
	 *                     perform editing operations
	 * @throws NullPointerException           if task number of given as empty
	 *                                        string or null
	 * @throws ArrayIndexOutOfBoundsException if task number does not fall in index
	 *                                        range of ArrayList
	 */
	public void editTask(String selectedTask) throws NullPointerException {
		try {
			...
			case "2":
				task.markCompleted();
				Messages.showMessage("Task Num " + selectedTask + " is marked as Completed: Returning to Main Menu",
						false);
				break;
			case "3":
	...
	/**
	 * This method will read the data file from disk which will contain the data of
	 * previously saved tasks
	 * 
	 * @param filename a string specifying the full path and extension of data file,
	 *                 for example, "resources/tasks.obj"
